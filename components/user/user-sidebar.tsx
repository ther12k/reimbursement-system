...
import { LayoutDashboard, FileText, PlusCircle, CalendarDays, Settings, LogOut, ChevronRight } from "lucide-react"
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
      ...
      <div className="fixed top-0 left-0 right-0 h-14 border-b border-border bg-background md:pl-[var(--sidebar-width)] md:group-data-[collapsible=icon]/sidebar-wrapper:pl-[var(--sidebar-width-icon)] z-10 transition-all duration-300 flex items-center px-4">
        ...
        <div className="flex-1 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">{routes.find((route) => route.active)?.label || "Pengguna"}</span>
          </div>
          <ThemeToggle />
        </div>
      </div>
